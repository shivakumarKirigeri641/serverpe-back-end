...
const sendAlertForFeedbackSMS = async (
  ...
  const fast2smsResp = await axios.get(
    `https://www.fast2sms.com/dev/bulkV2?authorization=${process.env.FAST2SMSAPIKEY}&route=dlt&sender_id=SRVRPE&message=205285&variables_values=${mobile_number}|${feedback_category}|${rating}&numbers=${process.env.MYOWNNUMBER}`
  );
