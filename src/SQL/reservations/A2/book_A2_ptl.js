...
const book_2a_ptl = async (
  ...
  try {
    ...
    for (let i = 0; i < result_passengerdata.rows.length; i++) {
      let ptl_count = result_seatsondate.rows[0].seat_count_2a_ptl;
      //BOOK GENERAL TICKET IF AVAILABLE
      if (0 < result_seatsondate.rows[0].seat_count_2a_ptl) {
        let seat_details = getCoachSeatAndBerth(ptl_count, "2A");
        ...
        result_seatsondate = await client.query(
          `update seatsondate set seat_count_2a_ptl=$1 where id=$2 returning *`,
