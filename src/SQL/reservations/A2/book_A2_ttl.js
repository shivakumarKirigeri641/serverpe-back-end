...
const book_2a_ttl = async (
  ...
  try {
    ...
    let seatsondate_raccount = result_seatsondate.rows[0].seat_count_2a_rac;
    for (let i = 0; i < result_passengerdata.rows.length; i++) {
      let ttl_count = result_seatsondate.rows[0].seat_count_2a_ttl;
      //BOOK GENERAL TICKET IF AVAILABLE
      if (0 < result_seatsondate.rows[0].seat_count_2a_ttl) {
        let seat_details = getCoachSeatAndBerth(ttl_count, "2A");
        ...
        result_seatsondate = await client.query(
          `update seatsondate set seat_count_2a_ttl=$1 where id=$2 returning *`,
