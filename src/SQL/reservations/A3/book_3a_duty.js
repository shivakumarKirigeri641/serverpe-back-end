...
const book_3a_duty = async (
  ...
  try {
    ...
    for (let i = 0; i < result_passengerdata.rows.length; i++) {
      let gen_count = result_seatsondate.rows[0].seat_count_3a_duty;
      //BOOK GENERAL TICKET IF AVAILABLE
      if (0 < result_seatsondate.rows[0].seat_count_3a_duty) {
        ...
        result_seatsondate = await client.query(
          `update seatsondate set seat_count_3a_duty=$1 where id=$2 returning *`,
