...
const book_general_ea = async (
  ...
  for (let i = 0; i < passengerdetails.rows.length; i++) {
    ...
    if (0 < result_seats_availability.rows[0].gen_count) {
      ...
      const seat_details = allocateSeat_ea("EA", current_gen_seat);
  ...
  //send sms
  /*const fastecmsResp = await axios.get(
    `https://www.fastecms.com/dev/bulkV2?authorization=${
      process.env.FASTecMSAPIKEY
  ...
  if (fastecmsResp.data && fastecmsResp.data.return) {
    //dont do anything
    console.log(fastecmsResp.data);
  } else {
...
module.exports = book_general_ea;
