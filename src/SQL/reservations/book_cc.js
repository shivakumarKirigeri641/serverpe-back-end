...
const allocateSeat_cc = require("../../utils/allocateSeat_cc");
...
const book_cc = async (
  ...
  for (let i = 0; i < passengerdetails.rows.length; i++) {
    ...
    let result_seats_availability = await client.query(
      `select *from seatsondate_cc where train_number=$1 and date_of_journey=$2 for update`,
      [
    ...
    switch (result_details.rows[0].type_code.toUpperCase()) {
      case "TTL":
        if (0 < result_seats_availability.rows[0].ttl_count) {
          ...
          const seat_details = allocateSeat_cc(
            "C",
            current_gen_seat,
          ...
          await client.query(
            `update seatsondate_cc set ttl_count = $1, total_seats=$2 where id=$3`,
            [
          ...
          await client.query(
            `insert into seatallocation_cc (fkpassengerdata, fk_seatsondate_cc, seat_sequence_number, seat_status, coach, berth, seat_number)values ($1,$2,$3,$4,$5,$6,$7) returning *`,
            [
      ...
      case "PTL":
        if (0 < result_seats_availability.rows[0].ptl_count) {
          ...
          //decrement the count of gen_seats
          await client.query(
            `update seatsondate_cc set ptl_count = $1, total_seats=$2 where id=$3`,
            [
      ...
      case "LADIES":
        if (0 < result_seats_availability.rows[0].ladies_count) {
          ...
          //decrement the count of gen_seats
          await client.query(
            `update seatsondate_cc set ladies_count = $1, total_seats=$2 where id=$3`,
            [
      ...
      case "SENIOR":
        if (0 < result_seats_availability.rows[0].senior_count) {
          ...
          //decrement the count of gen_seats
          await client.query(
            `update seatsondate_cc set senior_count = $1, total_seats=$2 where id=$3`,
            [
      ...
      case "GEN":
        if (0 < result_seats_availability.rows[0].gen_count) {
          ...
          //decrement the count of gen_seats
          await client.query(
            `update seatsondate_cc set gen_count = $1, total_seats=$2 where id=$3`,
            [
        ...
        //waiting list
          ...
          const result_fetchrac_count = await client.query(
            `select count(*) as count from seatallocation_cc where fk_seatsondate_cc=$1 and seat_status=$2`,
          ...
          //insert into seat alloation
          await client.query(
            `insert into seatallocation_cc (fkpassengerdata, fk_seatsondate_cc, seat_status, current_seat_status, updated_seat_status)values ($1,$2,$3,$4,$5) returning *`,
            [
              passengerdetails.rows[i].id,
              result_seats_availability.rows[0].id,
              "WTL",
  ...
  //send sms
  /*const fastccmsResp = await axios.get(
    `https://www.fastccms.com/dev/bulkV2?authorization=${
      process.env.FASTccMSAPIKEY
  ...
  if (fastccmsResp.data && fastccmsResp.data.return) {
    //dont do anything
    console.log(fastccmsResp.data);
  } else {
