...
const book_sl_seniors = async (
  ...
  try {
    ...
    for (let i = 0; i < result_passengerdata.rows.length; i++) {
      let gen_count = result_seatsondate.rows[0].seat_count_sl_seniors;
      //BOOK GENERAL TICKET IF AVAILABLE
      if (0 < result_seatsondate.rows[0].seat_count_sl_seniors) {
        ...
        result_seatsondate = await client.query(
          `update seatsondate set seat_count_sl_seniors=$1 where id=$2 returning *`,
...
module.exports = book_sl_seniors;
