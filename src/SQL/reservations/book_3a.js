...
const book_3a = async (
  ...
  for (let i = 0; i < passengerdetails.rows.length; i++) {
    ...
    let result_seats_availability = await client.query(
      `select *from seatsondate_3a where train_number=$1 and date_of_journey=$2 for update`,
      [
    ...
    switch (result_details.rows[0].type_code.toUpperCase()) {
      case "TTL":
        if (0 < result_seats_availability.rows[0].ttl_count) {
          ...
          await client.query(
            `update seatsondate_3a set ttl_count = $1, total_seats=$2 where id=$3`,
            [
          ...
          await client.query(
            `insert into seatallocation_3a (fkpassengerdata, fk_seatsondate_3a, seat_sequence_number, seat_status, coach, berth, seat_number)values ($1,$2,$3,$4,$5,$6,$7) returning *`,
            [
      ...
      case "PTL":
        if (0 < result_seats_availability.rows[0].ptl_count) {
          ...
          //decrement the count of gen_seats
          await client.query(
            `update seatsondate_3a set ptl_count = $1, total_seats=$2 where id=$3`,
            [
      ...
      case "LADIES":
        if (0 < result_seats_availability.rows[0].ladies_count) {
          ...
          //decrement the count of gen_seats
          await client.query(
            `update seatsondate_3a set ladies_count = $1, total_seats=$2 where id=$3`,
            [
      ...
      case "PWD":
        if (0 < result_seats_availability.rows[0].pwd_count) {
          ...
          //decrement the count of gen_seats
          await client.query(
            `update seatsondate_3a set pwd_count = $1, total_seats=$2 where id=$3`,
            [
      ...
      case "DUTY":
        if (0 < result_seats_availability.rows[0].duty_count) {
          ...
          //decrement the count of gen_seats
          await client.query(
            `update seatsondate_3a set duty_count = $1, total_seats=$2 where id=$3`,
            [
      ...
      case "SENIOR":
        if (0 < result_seats_availability.rows[0].senior_count) {
          ...
          //decrement the count of gen_seats
          await client.query(
            `update seatsondate_3a set senior_count = $1, total_seats=$2 where id=$3`,
            [
      ...
      case "GEN":
        if (0 < result_seats_availability.rows[0].gen_count) {
          ...
          //decrement the count of gen_seats
          await client.query(
            `update seatsondate_3a set gen_count = $1, total_seats=$2 where id=$3`,
            [
        ...
        //check if rac
          ...
          const result_fetchrac_count = await client.query(
            `select count(*) as count from seatallocation_3a where fk_seatsondate_3a=$1 and seat_status=$2`,
        ...
        //check if rac_share_count is present
          ...
          //decrement the count of rac_seats
          await client.query(
            `update seatsondate_3a set rac_share_count = $1 where id=$2`,
            [
