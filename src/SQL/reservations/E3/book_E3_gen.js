...
const book_e3_gen = async (
  ...
  try {
    ...
    for (let i = 0; i < result_passengerdata.rows.length; i++) {
      let gen_count = result_seatsondate.rows[0].seat_count_e3_gen;
      //BOOK GENERAL TICKET IF AVAILABLE
      if (0 < result_seatsondate.rows[0].seat_count_e3_gen) {
        ...
        result_seatsondate = await client.query(
          `update seatsondate set seat_count_e3_gen=$1 where id=$2 returning *`,
