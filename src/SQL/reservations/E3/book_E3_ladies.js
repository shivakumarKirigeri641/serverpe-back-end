...
const book_e3_ladies = async (
  ...
  try {
    ...
    let seatsondate_rae3ount = result_seatsondate.rows[0].seat_count_e3_rac;
    for (let i = 0; i < result_passengerdata.rows.length; i++) {
      let gen_count = result_seatsondate.rows[0].seat_count_e3_ladies;
      //BOOK GENERAL TICKET IF AVAILABLE
      if (0 < result_seatsondate.rows[0].seat_count_e3_ladies) {
        let seat_details = getCoachSeatAndBerth(gen_count, "e3");
        ...
        result_seatsondate = await client.query(
          `update seatsondate set seat_count_e3_ladies=$1 where id=$2 returning *`,
          [gen_count, result_seatsondate.rows[0].id]
        );
      }
      //BOOK RAC
      else if (0 < result_seatsondate.rows[0].seat_count_e3_rac) {
        pnr_status = "RAC";
        ...
        passenger_details.push(temp.rows[0]);
        seatsondate_rae3ount--;
        result_seatsondate = await client.query(
          `update seatsondate set seat_count_e3_rac=$1 where id=$2 returning *`,
          [seatsondate_rae3ount, result_seatsondate.rows[0].id]
        );
        //rac
      }
      //WAITING LIST
