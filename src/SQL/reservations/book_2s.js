...
const allocateSeat_2s = require("../../utils/allocateSeat_2S");
...
const book_2s = async (
  ...
  for (let i = 0; i < passengerdetails.rows.length; i++) {
    ...
    let result_seats_availability = await client.query(
      `select *from seatsondate_2s where train_number=$1 and date_of_journey=$2 for update`,
      [
    ...
    switch (type_code) {
      case "TTL":
        if (0 < result_seats_availability.rows[0].ttl_count) {
          ...
          await client.query(
            `update seatsondate_2s set ttl_count = $1, total_seats=$2 where id=$3`,
            [
      ...
      case "PTL":
        if (0 < result_seats_availability.rows[0].ptl_count) {
          ...
          //decrement the count of gen_seats
          await client.query(
            `update seatsondate_2s set ptl_count = $1, total_seats=$2 where id=$3`,
            [
      ...
      case "LADIES":
        if (0 < result_seats_availability.rows[0].ladies_count) {
          ...
          //decrement the count of gen_seats
          await client.query(
            `update seatsondate_2s set ladies_count = $1, total_seats=$2 where id=$3`,
            [
      ...
      case "SENIOR":
        if (0 < result_seats_availability.rows[0].senior_count) {
          ...
          //decrement the count of gen_seats
          await client.query(
            `update seatsondate_2s set senior_count = $1, total_seats=$2 where id=$3`,
            [
      ...
      case "GEN":
        if (0 < result_seats_availability.rows[0].gen_count) {
          ...
          //decrement the count of gen_seats
          await client.query(
            `update seatsondate_2s set gen_count = $1, total_seats=$2 where id=$3`,
            [
        ...
        //waiting list
          ...
          const result_fetchrac_count = await client.query(
            `select count(*) as count from seatallocation_2s where fk_seatsondate_2s=$1 and seat_status=$2`,
          ...
          //insert into seat alloation
          await client.query(
            `insert into seatallocation_2s (fkpassengerdata, fk_seatsondate_2s, seat_status, current_seat_status, updated_seat_status)values ($1,$2,$3,$4,$5) returning *`,
            [
              passengerdetails.rows[i].id,
              result_seats_availability.rows[0].id,
              "WTL",
