...
const book_cc_seniors = async (
  ...
  try {
    ...
    let seatsondate_raccount = result_seatsondate.rows[0].seat_count_cc_rac;
    for (let i = 0; i < result_passengerdata.rows.length; i++) {
      let gen_count = result_seatsondate.rows[0].seat_count_cc_seniors;
      //BOOK GENERAL TICKET IF AVAILABLE
      if (0 < result_seatsondate.rows[0].seat_count_cc_seniors) {
        let seat_details = getCoachSeatAndBerth(gen_count, "CC");
        ...
        result_seatsondate = await client.query(
          `update seatsondate set seat_count_cc_seniors=$1 where id=$2 returning *`,
          [gen_count, result_seatsondate.rows[0].id]
        );
      }
      //BOOK RAC
      else if (0 < result_seatsondate.rows[0].seat_count_cc_rac) {
        pnr_status = "RAC";
        ...
        result_seatsondate = await client.query(
          `update seatsondate set seat_count_cc_rac=$1 where id=$2 returning *`,
...
module.exports = book_cc_seniors;
