...
const book_ec_gen = async (
  ...
  try {
    ...
    for (let i = 0; i < result_passengerdata.rows.length; i++) {
      let gen_count = result_seatsondate.rows[0].seat_count_ec_gen;
      //BOOK GENERAL TICKET IF AVAILABLE
      if (0 < result_seatsondate.rows[0].seat_count_ec_gen) {
        let seat_details = getCoachSeatAndBerth(gen_count, "ec");
        ...
        result_seatsondate = await client.query(
          `update seatsondate set seat_count_ec_gen=$1 where id=$2 returning *`,
