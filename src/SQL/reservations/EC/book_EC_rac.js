...
const book_ec_rac = async (
  ...
  try {
    ...
    for (let i = 0; i < result_passengerdata.rows.length; i++) {
      let rac_count = result_seatsondate.rows[0].seat_count_ec_rac;
      //BOOK GENERAL TICKET IF AVAILABLE
      if (0 < result_seatsondate.rows[0].seat_count_ec_rac) {
        ...
        result_seatsondate = await client.query(
          `update seatsondate set seat_count_ec_rac=$1 where id=$2 returning *`,
