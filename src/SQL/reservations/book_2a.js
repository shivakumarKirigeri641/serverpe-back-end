    ...
    switch (type_code) {
      case "TTL":
        if (0 < result_seats_availability.rows[0].ttl_count) {
          ...
          await client.query(
            `update seatsondate_2a set ttl_count = $1, total_seats=$2 where id=$3`,
            [
      ...
      case "PTL":
        if (0 < result_seats_availability.rows[0].ptl_count) {
          ...
          //decrement the count of gen_seats
          await client.query(
            `update seatsondate_2a set ptl_count = $1, total_seats=$2 where id=$3`,
            [
      ...
      case "GEN":
        ...
        //check if rac
          ...
          await client.query(
            `insert into seatallocation_2a (fkpassengerdata, fk_seatsondate_2a, seat_status, current_seat_status, updated_seat_status, seat_sequence_number)values ($1,$2,$3,$4,$5,$6) returning *`,
            [
              passengerdetails.rows[i].id,
              result_seats_availability.rows[0].id,
              "RAC",
