...
const runSimulator_e3 = async (pool) => {
  const train_numbers_e3 = await pool.query(
    `SELECT train_number, date_of_journey
FROM (
  SELECT DISTINCT train_number, date_of_journey
  FROM seatsondate_e3 where date_of_journey >= (CURRENT_DATE + INTERVAL '1 day')
) AS distinct_data
ORDER BY RANDOM()
LIMIT 1;
`
  ...
  const { source_code, destination_code } = await getSourceAndDestination(
    pool,
    train_numbers_e3.rows[0].train_number
  );
  ...
  const dojDate = new Date(train_numbers_e3.rows[0].date_of_journey);
  ...
  const body = {
    train_number: train_numbers_e3.rows[0].train_number,
    doj: train_numbers_e3.rows[0].date_of_journey,
    coach_type: "E3",
...
module.exports = runSimulator_e3;
