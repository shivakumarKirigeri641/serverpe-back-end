...
const runSimulator_2a = async (pool) => {
  const train_numbers_2a = await pool.query(
    `SELECT train_number, date_of_journey
FROM (
  SELECT DISTINCT train_number, date_of_journey
  FROM seatsondate_2a where date_of_journey >= (CURRENT_DATE + INTERVAL '1 day')
) AS distinct_data
ORDER BY RANDOM()
LIMIT 1;
`
  ...
  const dojDate = new Date(train_numbers_2a.rows[0].date_of_journey);
