...
const getLicenseKey = require("../../utils/getLicenseKey");
const inserTransactions = async (
  ...
  ------------------------------------------*/
    await client.query(
      ...
      [order.id, transaction_data.order_id, transaction_data.id],
    );
  ...
  ------------------------------------------*/
    const licenseKey = getLicenseKey(user.id);

    // Generate API key only for UI_ONLY projects
    const apiKey = generateApiKey(user.id, order.order_number);
    ...
    return {
      ...
      result_transaction: {
        ...
        description:
          ...
          `Plan: ${summaryFormData?.project?.project_code}`,
        created_at:
          transaction_data.created_at || Math.floor(Date.now() / 1000),
