    ...
    return new Promise((resolve, reject) => {
        try {
            ...
            const invoicesDir = path.join(
                __dirname, 
                '../../secure-storage/dowloads/invoices',
                year.toString()
            );
            ...
            const relativePath = `src/secure-storage/dowloads/invoices/${year}/${fileName}`;
            ...
            doc.font('Helvetica').text(`Invoice No: ${invoiceData.invoice_number}`, rightX, y + 15);
            doc.text(`Date: ${invoiceDate}`, rightX, y + 28);
            doc.text(`Payment ID: ${invoiceData.payment_id || 'N/A'}`, rightX, y + 41);
            ...
            y = doc.y;
            doc.font('Helvetica');
            doc.text(invoiceData.project_title || 'Project Purchase', 50, y, { width: 250 });
            doc.text(invoiceData.project_type || 'FULL_STACK', 300, y, { width: 80 });
            doc.text(Number(invoiceData.taxable_amount).toFixed(2), 400, y, { width: 100, align: 'right' });
            ...
            doc.font('Helvetica')
               ...
               .text(`₹ ${Number(invoiceData.taxable_amount).toFixed(2)}`, amountX, y, { align: 'right' });

            y += 18;
            doc.text('CGST (9%):', totalsX, y)
               .text(`₹ ${(Number(invoiceData.gst_amount) / 2).toFixed(2)}`, amountX, y, { align: 'right' });
            ...
            doc.font('Helvetica-Bold')
               ...
               .text(`₹ ${Number(invoiceData.total_amount).toFixed(2)}`, amountX, y, { align: 'right' });
            ...
            doc.text('This is a computer generated invoice and does not require signature.', 50, doc.y, { align: 'center' });
            doc.moveDown();
            doc.text('For support: support@serverpe.in | www.serverpe.in', { align: 'center' });
